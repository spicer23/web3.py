...
def fixture_middleware():
    return construct_fixture_middleware({
        'fake_endpoint': lambda *_: {'result': str(uuid.uuid4())},
        'not_whitelisted': lambda *_: {'result': str(uuid.uuid4())},
    })


@pytest.fixture
def w3(w3_base, fixture_middleware):
    w3_base.middleware_stack.add(fixture_middleware)
    return w3_base
...
def test_simple_cache_middleware_does_not_cache_none_responses(w3_base):
    ...
    w3 = w3_base
    w3.middleware_stack.add(construct_fixture_middleware({
        'fake_endpoint': lambda *_: {'result': None, 'id': next(counter)},
    }))
...
def test_simple_cache_middleware_does_not_cache_error_responses(w3_base):
    w3 = w3_base
    w3.middleware_stack.add(construct_fixture_middleware({
        'fake_endpoint': lambda *_: {'error': 'msg-{0}'.format(str(uuid.uuid4()))},
    }))
